...
static inline int ibx_msg(const char *fmt, ...) {
  ...
  if (result != -1) {
    result = fprintf(stderr, "%s %s: %s", date, INNOBACKUPEX_BIN_NAME, line);
    free(line);
  }
...
static struct my_option ibx_long_options[] = {
    {"version", 'v', "print xtrabackup version information",
     (uchar *)&opt_ibx_version, (uchar *)&opt_ibx_version, 0, GET_BOOL, NO_ARG,
     0, 0, 0, 0, 0, 0},

    {"help", '?', "This option displays a help screen and exits.",
     (uchar *)&opt_ibx_help, (uchar *)&opt_ibx_help, 0, GET_BOOL, NO_ARG, 0, 0,
     0, 0, 0, 0},

    {"apply-log", OPT_APPLY_LOG,
     "Prepare a backup in BACKUP-DIR by "
     "applying the redo log 'ib_logfile0' and creating new redo log. "
     "The InnoDB configuration is read from the file \"backup-my.cnf\".",
     (uchar *)&opt_ibx_apply_log, (uchar *)&opt_ibx_apply_log, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},

    {"copy-back", OPT_COPY_BACK,
     "Copy all the files in a previously made "
     "backup from the backup directory to their original locations.",
     (uchar *)&opt_ibx_copy_back, (uchar *)&opt_ibx_copy_back, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"move-back", OPT_MOVE_BACK,
     ...
     (uchar *)&opt_ibx_move_back, (uchar *)&opt_ibx_move_back, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"galera-info", OPT_GALERA_INFO,
     ...
     (uchar *)&opt_ibx_galera_info, (uchar *)&opt_ibx_galera_info, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"slave-info", OPT_SLAVE_INFO,
     ...
     (uchar *)&opt_ibx_slave_info, (uchar *)&opt_ibx_slave_info, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"incremental", OPT_INCREMENTAL,
     ...
     (uchar *)&opt_ibx_incremental, (uchar *)&opt_ibx_incremental, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"no-lock", OPT_NO_LOCK,
     ...
     "using this option.",
     (uchar *)&opt_ibx_no_lock, (uchar *)&opt_ibx_no_lock, 0, GET_BOOL, NO_ARG,
     0, 0, 0, 0, 0, 0},
    ...
    {"rsync", OPT_RSYNC,
     ...
     "--stream.",
     (uchar *)&opt_ibx_rsync, (uchar *)&opt_ibx_rsync, 0, GET_BOOL, NO_ARG, 0,
     0, 0, 0, 0, 0},
    ...
    {"force-non-empty-directories", OPT_FORCE_NON_EMPTY_DIRS,
     ...
     (uchar *)&opt_ibx_force_non_empty_dirs, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
     0},

    {"no-timestamp", OPT_NO_TIMESTAMP,
     "This option prevents creation of a "
     ...
     (uchar *)&opt_ibx_notimestamp, (uchar *)&opt_ibx_notimestamp, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},

    {"no-version-check", OPT_NO_VERSION_CHECK,
     "This option disables the "
     "version check which is enabled by the --version-check option.",
     (uchar *)&opt_ibx_noversioncheck, (uchar *)&opt_ibx_noversioncheck, 0,
    ...
    {"no-backup-locks", OPT_NO_BACKUP_LOCKS,
     ...
     (uchar *)&opt_ibx_no_backup_locks, (uchar *)&opt_ibx_no_backup_locks, 0,
     GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},

    {"decompress", OPT_DECOMPRESS,
     "Decompresses all files with the .qp "
     "extension in a backup previously made with the --compress option.",
     (uchar *)&opt_ibx_decompress, (uchar *)&opt_ibx_decompress, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"user", 'u',
     ...
     (uchar *)&opt_ibx_user, (uchar *)&opt_ibx_user, 0, GET_STR, REQUIRED_ARG,
     0, 0, 0, 0, 0, 0},
    ...
    {"host", 'H',
     ...
     (uchar *)&opt_ibx_host, (uchar *)&opt_ibx_host, 0, GET_STR, REQUIRED_ARG,
     0, 0, 0, 0, 0, 0},
    ...
    {"port", 'P',
     ...
     "connecting to the database server with TCP/IP.  The option accepts "
     "a string argument. See mysql --help for details.",
     &opt_ibx_port, &opt_ibx_port, 0, GET_UINT, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"incremental-history-name", OPT_INCREMENTAL_HISTORY_NAME,
     ...
     (uchar *)&opt_ibx_incremental_history_name, 0, GET_STR, REQUIRED_ARG, 0, 0,
     0, 0, 0, 0},
    ...
    {"incremental-history-uuid", OPT_INCREMENTAL_HISTORY_UUID,
     ...
     (uchar *)&opt_ibx_incremental_history_uuid, 0, GET_STR, REQUIRED_ARG, 0, 0,
     0, 0, 0, 0},
    ...
    {"ftwrl-wait-query-type", OPT_LOCK_WAIT_QUERY_TYPE,
     ...
     (uchar *)&opt_ibx_lock_wait_query_type, &query_type_typelib, GET_ENUM,
    ...
    {"kill-long-query-type", OPT_KILL_LONG_QUERY_TYPE,
     ...
     (uchar *)&opt_ibx_kill_long_query_type, &query_type_typelib, GET_ENUM,
     REQUIRED_ARG, QUERY_TYPE_SELECT, 0, 0, 0, 0, 0},
    ...
    {"include", OPT_INCLUDE,
     ...
     "details.",
     (uchar *)&opt_ibx_include, (uchar *)&opt_ibx_include, 0, GET_STR,
     REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"databases", OPT_DATABASES,
     ...
     (uchar *)&opt_ibx_databases, (uchar *)&opt_ibx_databases, 0, GET_STR,
     REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"kill-long-queries-timeout", OPT_KILL_LONG_QUERIES_TIMEOUT,
     ...
     (uchar *)&opt_ibx_kill_long_queries_timeout, 0, GET_UINT, REQUIRED_ARG, 0,
     0, 0, 0, 0, 0},
    ...
    {"ftwrl-wait-threshold", OPT_LOCK_WAIT_THRESHOLD,
     ...
     (uchar *)&opt_ibx_lock_wait_threshold, 0, GET_UINT, REQUIRED_ARG, 60, 0, 0,
     0, 0, 0},

    {"debug-sleep-before-unlock", OPT_DEBUG_SLEEP_BEFORE_UNLOCK,
     "This is a debug-only option used by the XtraBackup test suite.",
     (uchar *)&opt_ibx_debug_sleep_before_unlock,
     (uchar *)&opt_ibx_debug_sleep_before_unlock, 0, GET_UINT, REQUIRED_ARG, 0,
     0, 0, 0, 0, 0},
    ...
    {"safe-slave-backup-timeout", OPT_SAFE_SLAVE_BACKUP_TIMEOUT,
     ...
     (uchar *)&opt_ibx_safe_slave_backup_timeout, 0, GET_UINT, REQUIRED_ARG,
     300, 0, 0, 0, 0, 0},
    ...
    {"close_files", OPT_CLOSE_FILES,
     "Do not keep files opened. This "
     "option is passed directly to xtrabackup. Use at your own risk.",
     (uchar *)&ibx_xb_close_files, (uchar *)&ibx_xb_close_files, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"compress", OPT_COMPRESS,
     ...
     (uchar *)&ibx_xtrabackup_compress_alg, 0, GET_STR, OPT_ARG, 0, 0, 0, 0, 0,
     0},
    ...
    {"compress-threads", OPT_COMPRESS_THREADS,
     ...
     (uchar *)&ibx_xtrabackup_compress_threads, 0, GET_UINT, REQUIRED_ARG, 1, 1,
     UINT_MAX, 0, 0, 0},

    {"compress-chunk-size", OPT_COMPRESS_CHUNK_SIZE,
     "Size of working "
     "buffer(s) for compression threads in bytes. The default value "
     "is 64K.",
     (uchar *)&ibx_xtrabackup_compress_chunk_size,
     (uchar *)&ibx_xtrabackup_compress_chunk_size, 0, GET_ULL, REQUIRED_ARG,
    ...
    {"export", OPT_EXPORT,
     ...
     (uchar *)&ibx_xtrabackup_export, (uchar *)&ibx_xtrabackup_export, 0,
     GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},

    {"extra-lsndir", OPT_EXTRA_LSNDIR,
     ...
     (uchar *)&ibx_xtrabackup_extra_lsndir, 0, GET_STR, REQUIRED_ARG, 0, 0, 0,
     0, 0, 0},

    {"incremental-basedir", OPT_INCREMENTAL_BASEDIR,
     "This option "
     ...
     (uchar *)&ibx_xtrabackup_incremental_basedir, 0, GET_STR, REQUIRED_ARG, 0,
     0, 0, 0, 0, 0},

    {"incremental-dir", OPT_INCREMENTAL_DIR,
     "This option specifies the "
     ...
     (uchar *)&ibx_xtrabackup_incremental_dir, 0, GET_STR, REQUIRED_ARG, 0, 0,
     0, 0, 0, 0},
    ...
    {"incremental-force-scan", OPT_INCREMENTAL_FORCE_SCAN,
     ...
     (uchar *)&ibx_xtrabackup_incremental_force_scan, 0, GET_BOOL, NO_ARG, 0, 0,
     0, 0, 0, 0},

    {"log-copy-interval", OPT_LOG_COPY_INTERVAL,
     "This option specifies "
     "time interval between checks done by log copying thread in "
     "milliseconds.",
     (uchar *)&ibx_xtrabackup_log_copy_interval,
     (uchar *)&ibx_xtrabackup_log_copy_interval, 0, GET_LONG, REQUIRED_ARG,
     1000, 0, LONG_MAX, 0, 1, 0},

    {"incremental-lsn", OPT_INCREMENTAL,
     "This option specifies the log "
     ...
     "64-bit integer.",
     (uchar *)&ibx_xtrabackup_incremental, (uchar *)&ibx_xtrabackup_incremental,
    ...
    {"parallel", OPT_PARALLEL,
     ...
     (uchar *)&ibx_xtrabackup_parallel, (uchar *)&ibx_xtrabackup_parallel, 0,
    ...
    {"throttle", OPT_THROTTLE,
     ...
     (uchar *)&ibx_xtrabackup_throttle, (uchar *)&ibx_xtrabackup_throttle, 0,
     GET_LONG, REQUIRED_ARG, 0, 0, LONG_MAX, 0, 1, 0},

    {"tmpdir", 't',
     "This option specifies the location where a temporary "
     ...
     (uchar *)&ibx_opt_mysql_tmpdir, (uchar *)&ibx_opt_mysql_tmpdir, 0, GET_STR,
     REQUIRED_ARG, 0, 0, 0, 0, 0, 0},

    {"use-memory", OPT_USE_MEMORY,
...
static my_bool ibx_get_one_option(int optid,
  ...
  switch (optid) {
    case '?':
      ...
      break;
    case 'v':
      printf("innobackupex version %s %s (%s)", MYSQL_SERVER_VERSION,
...
bool make_backup_dir() {
  time_t t = time(NULL);
  char buf[100];

  if (!opt_ibx_notimestamp && !ibx_xtrabackup_stream_str) {
    strftime(buf, sizeof(buf), "%Y-%m-%d_%H-%M-%S", localtime(&t));
    ut_a(asprintf(&ibx_backup_directory, "%s/%s", ibx_position_arg, buf) != -1);
  } else {
...
bool ibx_handle_options(int *argc, char ***argv) {
  ...
  while (i < *argc) {
    ...
    if (strncmp(opt, "--", 2) != 0 && !(strlen(opt) == 2 && opt[0] == '-')) {
      if (ibx_position_arg != NULL && ibx_position_arg != opt) {
...
bool ibx_init() {
  ...
  xtrabackup_incremental_force_scan = ibx_xtrabackup_incremental_force_scan;
  ...
  if (!opt_ibx_incremental &&
      (xtrabackup_incremental || xtrabackup_incremental_basedir ||
       opt_ibx_incremental_history_name || opt_ibx_incremental_history_uuid)) {
    ibx_msg(
        "Error: --incremental-lsn, --incremental-basedir, "
...
void ibx_cleanup() { free(ibx_backup_directory); }
