...
/**********************************************************************/ /**
 Closes a file. */
static void xb_fil_node_close_file(
    /*===================*/
...
xb_fil_cur_result_t xb_fil_cur_open(
  ...
  if (!node->is_open()) {
    ut_ad(cursor->is_system() || srv_operation == SRV_OPERATION_RESTORE_DELTA ||
          xb_close_files);

    node->handle = os_file_create_simple_no_error_handling(
        0, node->name, OS_FILE_OPEN, OS_FILE_READ_ALLOW_DELETE, true, &success);
    if (!success) {
...
#ifdef _WIN32
  HANDLE hDup;
  DuplicateHandle(GetCurrentProcess(), cursor->file.m_file, GetCurrentProcess(),
  ...
  if (err) {
    msg(thread_n, "mariabackup: error: cannot fstat %s", cursor->abs_path);
  ...
  cursor->orig_buf =
      static_cast<byte *>(malloc(cursor->buf_size + srv_page_size));
  cursor->buf = static_cast<byte *>(ut_align(cursor->orig_buf, srv_page_size));
  ...
  if (!node->space->crypt_data &&
      os_file_read(IORequestRead, node->handle, cursor->buf, 0,
                   cursor->page_size)) {
    mutex_enter(&fil_system.mutex);
    if (!node->space->crypt_data) {
      node->space->crypt_data =
          fil_space_read_crypt_data(node->space->zip_size(), cursor->buf);
    }
    mutex_exit(&fil_system.mutex);
  }

  cursor->space_size = (ulint)(cursor->statinfo.st_size / cursor->page_size);
...
static bool page_is_corrupted(const byte *page, ulint page_no,
  ...
  if (cursor->space_id == TRX_SYS_SPACE && page_no >= FSP_EXTENT_SIZE &&
  ...
  if (mach_read_from_4(page + FIL_PAGE_OFFSET) != page_no &&
    ...
    const ulint *const end = reinterpret_cast<const ulint *>(page + page_size);
    do {
      if (*p++) {
        return true;
      }
  ...
  if (page_no &&
    ...
    if (!fil_space_verify_crypt_checksum(page, space->zip_size())) return true;
    ...
    if (!space->crypt_data ||
        space->crypt_data->type == CRYPT_SCHEME_UNENCRYPTED ||
        !fil_space_decrypt(space, tmp_frame, tmp_page, &decrypted)) {
      return true;
    }
  ...
  if (page_type == FIL_PAGE_PAGE_COMPRESSED ||
    ...
    return (!decomp || (decomp != srv_page_size && cursor->zip_size) ||
  ...
  if ((to_read & ~(page_size - 1)) &&
    ...
    if (to_read < (ib_int64_t)page_size) {
      msg(cursor->thread_n,
          ...
          cursor->abs_path, (ulonglong)offset, (ulonglong)to_read);
      return (XB_FIL_CUR_EOF);
    }

    to_read = (ib_int64_t)(((ulint)to_read) & ~(page_size - 1));
  }
