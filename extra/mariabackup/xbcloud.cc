...
enum { SWIFT, S3 };
const char *storage_names[] = {"SWIFT", "S3", NullS};
...
static struct my_option my_long_options[] = {
    ...
    {"swift-domain", OPT_SWIFT_DOMAIN, "The domain name.", &opt_swift_domain,
     &opt_swift_domain, 0, GET_STR_ALLOC, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},

    {"swift-domain-id", OPT_SWIFT_DOMAIN_ID, "The domain ID.",
    ...
    {"parallel", OPT_PARALLEL, "Number of parallel chunk uploads.",
     &opt_parallel, &opt_parallel, 0, GET_ULONG, REQUIRED_ARG, 1, 0, 0, 0, 0,
     0},
...
static const char *const masked_args[] = {
    "--swift-password",  "--swift-key",
    "--swift-auth-url",  "--swift-storage-url",
    "--swift-container", "--swift-user",
    "--swift-tenant",    "--swift-user-id",
    "--swift-tenant-id", 0};
...
static void usage() {
  ...
  puts("Usage: ");
  printf(
      "  %s -c put [OPTIONS...] <NAME> upload backup from STDIN into "
      "the cloud service with given name.\n",
      my_progname);
  printf(
      "  %s -c get [OPTIONS...] <NAME> [FILES...] stream specified "
...
static void mask_args(int argc, char **argv) {
  int i;
  for (i = 0; i < argc - 1; i++) {
    int j = 0;
    if (argv[i])
...
static int parse_args(int argc, char **argv) {
  ...
  if (opt_storage == SWIFT) {
    ...
    if (opt_swift_container == NULL) {
      fprintf(stderr, "Swift container is not specified\n");
      exit(EXIT_FAILURE);
    }
...
static int get_http_header(const char *prefix, const char *buffer, char *out,
                           size_t out_size) {
  const char *beg, *end;
...
static size_t swift_auth_header_read_cb(char *ptr, size_t size, size_t nmemb,
                                        void *data) {
  swift_auth_info *info = (swift_auth_info *)(data);

  get_http_header("X-Storage-Url: ", ptr, info->url, array_elements(info->url));
  get_http_header("X-Auth-Token: ", ptr, info->token,
                  array_elements(info->token));
...
static bool swift_temp_auth(const char *auth_url, swift_auth_info *info) {
  CURL *curl;
  CURLcode res;
  long http_code;
  char *hdr_buf = NULL;
  ...
  if (curl != NULL) {
    hdr_buf = (char *)(calloc(
        14 + max(strlen(opt_swift_user), strlen(opt_swift_key)), 1));
    ...
    curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, swift_auth_header_read_cb);
    curl_easy_setopt(curl, CURLOPT_HEADERDATA, info);
    curl_easy_setopt(curl, CURLOPT_HTTPHEADER, slist);
    if (opt_cacert != NULL) curl_easy_setopt(curl, CURLOPT_CAINFO, opt_cacert);
    if (opt_insecure) curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, FALSE);
...
static size_t write_null_cb(char *buffer, size_t size, size_t nmemb,
                            void *stream) {
  return fwrite(buffer, size, nmemb, stderr);
}

static size_t read_null_cb(char *ptr, size_t size, size_t nmemb, void *data) {
  return 0;
}

static int swift_create_container(swift_auth_info *info, const char *name) {
  char url[SWIFT_MAX_URL_SIZE];
...
static bool swift_delete_object(swift_auth_info *info, const char *url) {
  char auth_token[SWIFT_MAX_HDR_SIZE];
  CURLcode res;
  long http_code;
  CURL *curl;
    ...
    if (msg->msg == CURLMSG_DONE) {
      ...
      if (conn->chunk_acked) {
        ...
        fprintf(stderr,
                ...
                conn->chunk_size - conn->upload_size, conn->chunk_size);
...
static void setsock(socket_info *fdp, curl_socket_t s, CURL *easy, int action,
  ...
  fdp->action = action;
  fdp->easy = easy;
  if (fdp->evset) ev_io_stop(global->loop, &fdp->ev);
...
static connection_info *get_current_connection(global_io_info *global) {
  ...
  if (conn && conn->filled_size < conn->chunk_size) return conn;
...
static void input_cb(EV_P_ struct ev_io *w, int revents) {
  ...
  if (conn->filled_size < conn->chunk_size) {
    if (revents & EV_READ) {
      ssize_t nbytes =
          read(io_global->input_fd, conn->buffer + conn->filled_size,
      ...
      if (nbytes > 0) {
        ...
        if (errno != EAGAIN && errno != EINTR) {
          char error[200];
          my_strerror(error, sizeof(error), errno);
          fprintf(stderr,
...
static int swift_upload_read_cb(char *ptr, size_t size, size_t nmemb,
  ...
  if (conn->filled_size == conn->upload_size &&
    ...
    do {
      nbytes = read(conn->global->input_fd, conn->buffer + conn->filled_size,
                    conn->chunk_size - conn->filled_size);
...
static size_t upload_header_read_cb(char *ptr, size_t size, size_t nmemb,
                                    void *data) {
  connection_info *conn = (connection_info *)(data);
  char etag[33];
...
static int conn_upload_start(connection_info *conn) {
  ...
  fprintf(stderr,
          "uploading chunk %s/%s/%s.%020zu "
          "(md5: %s, size: %zu)\n",
          global->container, global->backup_name, conn->name, conn->chunk_no,
          conn->hash, conn->chunk_size);

  snprintf(object_url, array_elements(object_url), "%s/%s/%s/%s.%020zu",
           global->url, global->container, global->backup_name, conn->name,
           conn->chunk_no);
  ...
  snprintf(token_header, array_elements(token_header), "X-Auth-Token: %s",
           global->token);

  conn->slist = curl_slist_append(conn->slist, token_header);
  conn->slist = curl_slist_append(conn->slist, "Connection: keep-alive");
  ...
  curl_easy_setopt(conn->easy, CURLOPT_INFILESIZE, (long)conn->chunk_size);
  if (opt_cacert != NULL)
    curl_easy_setopt(conn->easy, CURLOPT_CAINFO, opt_cacert);
  if (opt_insecure) curl_easy_setopt(conn->easy, CURLOPT_SSL_VERIFYPEER, FALSE);
...
#if (OLD_CURL_MULTI)
  do {
    rc = curl_multi_socket_all(global->multi, &global->still_running);
  } while (rc == CURLM_CALL_MULTI_PERFORM);
#endif
...
static connection_info *conn_new(global_io_info *global, ulong global_idx) {
  ...
  if ((conn->buffer = (char *)(calloc(conn->buffer_size, 1))) == NULL) {
    goto error;
  }
...
static void conn_buffer_updated(connection_info *conn) {
  ...
  /* chunk header */
  if (!conn->magic_verified && conn->filled_size >= CHUNK_HEADER_CONSTANT_LEN) {
    ...
    conn->chunk_path_len = uint4korr(conn->buffer + PATH_LENGTH_OFFSET);
    conn->chunk_type = (xb_chunk_type_t)(conn->buffer[CHUNK_TYPE_OFFSET]);
    conn->chunk_size = CHUNK_HEADER_CONSTANT_LEN + conn->chunk_path_len;
  ...
  /* ordinary chunk */
  if (conn->magic_verified && conn->payload_size == 0 &&
      conn->chunk_type != XB_CHUNK_TYPE_EOF &&
      conn->filled_size >=
          CHUNK_HEADER_CONSTANT_LEN + conn->chunk_path_len + 16) {
    conn->payload_size = uint8korr(conn->buffer + CHUNK_HEADER_CONSTANT_LEN +
                                   conn->chunk_path_len);

    conn->chunk_size = conn->payload_size + 4 + 16 + conn->chunk_path_len +
                       CHUNK_HEADER_CONSTANT_LEN;

    if (conn->name == NULL) {
      conn->name = (char *)(malloc(conn->chunk_path_len + 1));
    } else if (conn->name_len < conn->chunk_path_len + 1) {
      conn->name = (char *)(realloc(conn->name, conn->chunk_path_len + 1));
    }
    ...
    if (conn->buffer_size < conn->chunk_size) {
      conn->buffer = (char *)(realloc(conn->buffer, conn->chunk_size));
      conn->buffer_size = conn->chunk_size;
    }
  }

  /* EOF chunk has no payload */
  if (conn->magic_verified && conn->chunk_type == XB_CHUNK_TYPE_EOF &&
      conn->filled_size >= CHUNK_HEADER_CONSTANT_LEN + conn->chunk_path_len) {
    if (conn->name == NULL) {
      conn->name = (char *)(malloc(conn->chunk_path_len + 1));
    } else if (conn->name_len < conn->chunk_path_len + 1) {
      conn->name = (char *)(realloc(conn->name, conn->chunk_path_len + 1));
...
static int swift_upload_parts(swift_auth_info *auth, const char *container,
                              const char *name) {
  ...
  io_global.connections =
      (connection_info **)(calloc(opt_parallel, sizeof(connection_info)));
  ...
  for (i = 0; i < opt_parallel; i++) {
    ...
    if (conn && conn->upload_size != conn->filled_size) {
      fprintf(stderr,
              "error: upload failed: %lu bytes left "
              "in the buffer %s (uploaded = %d)\n",
              (ulong)(conn->filled_size - conn->upload_size), conn->name,
              conn->chunk_uploaded);
      ++n_dirty_buffers;
    }
  }
...
static size_t fetch_buffer_header_cb(char *ptr, size_t size, size_t nmemb,
                                     void *data) {
  ...
  if (get_http_header("Content-Length: ", ptr, content_length_str,
    ...
    if (buf_size > buffer_info->size) {
      buffer_info->buf = (char *)(realloc(buffer_info->buf, buf_size));
      buffer_info->size = buf_size;
    }
  ...
  if (buffer_info->custom_header_callback) {
    buffer_info->custom_header_callback(
        ptr, size, nmemb, buffer_info->custom_header_callback_data);
  }
...
static size_t fetch_buffer_cb(char *buffer, size_t size, size_t nmemb,
                              void *out_buffer) {
...
static char *swift_fetch_into_buffer(swift_auth_info *auth, const char *url,
  ...
  if (curl != NULL) {
    ...
    curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, fetch_buffer_header_cb);
    curl_easy_setopt(curl, CURLOPT_HEADERDATA, &buffer_info);
    if (opt_cacert != NULL) curl_easy_setopt(curl, CURLOPT_CAINFO, opt_cacert);
    if (opt_insecure) curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, FALSE);
...
static container_list *container_list_new() {
  container_list *list = (container_list *)(calloc(1, sizeof(container_list)));
...
static void container_list_free(container_list *list) {
  free(list->content_json);
  free(list->objects);
  free(list);
}

static void container_list_add_object(container_list *list, const char *name,
                                      const char *hash, size_t bytes) {
  const size_t object_count_step = 1000;

  if (list->idx >= list->object_count) {
    list->objects = (object_info *)realloc(
        list->objects,
        (list->object_count + object_count_step) * sizeof(object_info));
...
jsmntok_t *json_tokenise(char *json, size_t len, int initial_tokens) {
  jsmn_parser parser;
  jsmn_init(&parser);
...
static bool json_token_eq(const char *buf, jsmntok_t *t, const char *s) {
  size_t len = strlen(s);
...
static bool json_token_str(const char *buf, jsmntok_t *t, char *out,
                           int out_size) {
...
static bool swift_parse_container_list(container_list *list) {
  enum { MAX_DEPTH = 20 };
  ...
  for (size_t i = 0, j = 1; j > 0; i++, j--) {
      ...
      case JSMN_STRING:
        if (stack[level].t->type == JSMN_OBJECT &&
          ...
          if (json_token_eq(response, t, "name")) {
            json_token_str(response, &tokens[i + 1], name, sizeof(name));
          }
          if (json_token_eq(response, t, "hash")) {
            json_token_str(response, &tokens[i + 1], hash, sizeof(hash));
          }
          if (json_token_eq(response, t, "bytes")) {
            json_token_str(response, &tokens[i + 1], bytes, sizeof(bytes));
          }
        }
        break;
    }
    ...
    while (stack[level].n_items == 0 && level > 0) {
      if (stack[level].t->type == JSMN_OBJECT && level == 2) {
        char *endptr;
...
/*********************************************************************/ /**
 ...
 object name. */
static container_list *swift_list(swift_auth_info *auth, const char *container,
                                  const char *path) {
  container_list *list;
  char url[SWIFT_MAX_URL_SIZE];
  ...
  while (!list->final) {
    ...
    snprintf(url, array_elements(url), "%s/%s?format=json&limit=1000%s%s%s%s",
             auth->url, container, path ? "&prefix=" : "", path ? path : "",
             list->idx > 0 ? "&marker=" : "",
             list->idx > 0 ? list->objects[list->idx - 1].name : "");

    list->content_json = swift_fetch_into_buffer(
        auth, url, &list->content_json, &list->content_bufsize,
...
static bool chunk_belongs_to(const char *chunk_name, const char *backup_name) {
...
static bool chunk_in_list(const char *chunk_name, char **list, int list_size) {
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if ((strncmp(chunk_name - item_len + chunk_name_len - 21, list[i],
                 item_len) == 0) &&
...
static int swift_download(swift_auth_info *auth, const char *container,
                          const char *name) {
  container_list *list;
  char *buf = NULL;
  size_t buf_size = 0;
  ...
  for (size_t i = 0; i < list->idx; i++) {
    ...
    if (chunk_belongs_to(chunk_name, name) &&
      ...
      snprintf(url, sizeof(url), "%s/%s/%s", auth->url, container, chunk_name);

      if ((buf = swift_fetch_into_buffer(auth, url, &buf, &buf_size,
                                         &result_len, NULL, NULL)) == NULL) {
        fprintf(stderr,
...
static bool swift_delete(swift_auth_info *auth, const char *container,
                         const char *name) {
  container_list *list;
...
static bool swift_backup_exists(swift_auth_info *auth, const char *container,
                                const char *backup_name) {
  container_list *list;
...
static bool swift_parse_keystone_response_v2(char *response,
  ...
  for (size_t i = 0, j = 1; j > 0; i++, j--) {
    ...
    switch (t->type) {
      ...
      case JSMN_STRING:
        if (stack[level].t->type == JSMN_OBJECT &&
          ...
          if (json_token_eq(response, t, "serviceCatalog")) {
          ...
          if (json_token_eq(response, t, "id")) {
            json_token_str(response, &tokens[i + 1], id, sizeof(id));
          }
          if (json_token_eq(response, t, "id") &&
              stack[level - 1].label == TOKEN) {
            json_token_str(response, &tokens[i + 1], token_id,
                           sizeof(token_id));
          }
          if (json_token_eq(response, t, "region")) {
            json_token_str(response, &tokens[i + 1], region, sizeof(region));
          }
          if (json_token_eq(response, t, "publicURL")) {
            json_token_str(response, &tokens[i + 1], public_url,
                           sizeof(public_url));
          }
          if (json_token_eq(response, t, "type")) {
            json_token_str(response, &tokens[i + 1], type, sizeof(type));
          }
        }
        break;
    }
    ...
    while (stack[level].n_items == 0 && level > 0) {
      if (stack[level].t->type == JSMN_OBJECT && level == 6 &&
          stack[level - 1].t->type == JSMN_ARRAY &&
          stack[level - 2].label == ENDPOINTS) {
        if (opt_swift_region == NULL || strcmp(opt_swift_region, region) == 0) {
          strncpy(filtered_url, public_url, sizeof(filtered_url));
        }
      }
      if (stack[level].t->type == JSMN_OBJECT && level == 4 &&
          stack[level - 1].t->type == JSMN_ARRAY &&
          stack[level - 2].label == CATALOG) {
        if (strcmp(type, "object-store") == 0) {
          strncpy(auth_info->url, filtered_url, sizeof(auth_info->url));
        }
      }
      --level;
    }
...
/*********************************************************************/ /**
 ...
 Uses creadentials privided as global variables.
 @returns true if access is granted and token received. */
static bool swift_keystone_auth_v2(const char *auth_url,
  ...
  if (opt_swift_user == NULL) {
    fprintf(stderr,
            "error: both --swift-user is required "
  ...
  if (opt_swift_password == NULL) {
    fprintf(stderr,
            "error: both --swift-password is required "
            "for keystone authentication.\n");
    return (false);
  }
  ...
  if (opt_swift_tenant != NULL) {
    snprintf(tenant_arg, sizeof(tenant_arg), ",\"%s\":\"%s\"", "tenantName",
             opt_swift_tenant);
  } else if (opt_swift_tenant_id != NULL) {
    snprintf(tenant_arg, sizeof(tenant_arg), ",\"%s\":\"%s\"", "tenantId",
             opt_swift_tenant_id);
  } else {
    *tenant_arg = 0;
  }
  ...
  if (curl != NULL) {
    ...
    curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, fetch_buffer_header_cb);
    curl_easy_setopt(curl, CURLOPT_HEADERDATA, &buf_info);
    curl_easy_setopt(curl, CURLOPT_HTTPHEADER, slist);
  ...
  if (!swift_parse_keystone_response_v2(buf_info.buf, buf_info.size, info)) {
    goto cleanup;
  }
...
/*********************************************************************/ /**
 Fills auth_info with response from keystone response.
 @return	true is response parsed successfully */
static bool swift_parse_keystone_response_v3(char *response,
                                             size_t response_length,
                                             swift_auth_info *auth_info) {
  enum { MAX_DEPTH = 20 };
    ...
    switch (t->type) {
      ...
      case JSMN_STRING:
        if (stack[level].t->type == JSMN_OBJECT &&
          ...
          if (json_token_eq(response, t, "catalog")) {
            stack[level].label = CATALOG;
          ...
          if (json_token_eq(response, t, "url")) {
            json_token_str(response, &tokens[i + 1], url, sizeof(url));
          }
          if (json_token_eq(response, t, "interface")) {
            json_token_str(response, &tokens[i + 1], interface,
                           sizeof(interface));
          }
          if (json_token_eq(response, t, "type")) {
            json_token_str(response, &tokens[i + 1], type, sizeof(type));
    ...
    while (stack[level].n_items == 0 && level > 0) {
      if (stack[level].t->type == JSMN_OBJECT && level == 6 &&
        ...
        if ((opt_swift_region == NULL ||
          ...
          strncpy(filtered_url, url, sizeof(filtered_url));
        }
      }
      if (stack[level].t->type == JSMN_OBJECT && level == 4 &&
          stack[level - 1].t->type == JSMN_ARRAY &&
...
static size_t keystone_v3_header_cb(char *ptr, size_t size, size_t nmemb,
                                    void *data) {
  swift_auth_info *info = (swift_auth_info *)(data);

  get_http_header("X-Subject-Token: ", ptr, info->token,
                  array_elements(info->token));
...
/*********************************************************************/ /**
 ...
 Uses creadentials privided as global variables.
 @returns true if access is granted and token received. */
static bool swift_keystone_auth_v3(const char *auth_url,
                                   swift_auth_info *info) {
  ...
  if (opt_swift_project_id != NULL && opt_swift_domain != NULL) {
    fprintf(stderr,
            "error: both --swift-project-id and "
  ...
  if (opt_swift_domain != NULL) {
    snprintf(domain, sizeof(domain), ",{\"domain\":{\"name\":\"%s\"}}",
             opt_swift_domain);
  } else if (opt_swift_domain_id != NULL) {
    snprintf(domain, sizeof(domain), ",{\"domain\":{\"id\":\"%s\"}}",
             opt_swift_domain_id);
  }

  if (opt_swift_project_id != NULL) {
    snprintf(scope, sizeof(scope), ",\"scope\":{\"project\":{\"id\":\"%s\"}}",
             opt_swift_project_id);
  ...
  if (!swift_parse_keystone_response_v3(buf_info.buf, buf_info.size, info)) {
    goto cleanup;
  }
...
int main(int argc, char **argv) {
  ...
  if (opt_swift_auth_version == NULL || *opt_swift_auth_version == '1') {
    /* TempAuth */
    snprintf(auth_url, SWIFT_MAX_URL_SIZE, "%sauth/v%s/", opt_swift_auth_url,
             opt_swift_auth_version ? opt_swift_auth_version : "1.0");
    ...
    /* Keystone v2 */
    snprintf(auth_url, SWIFT_MAX_URL_SIZE, "%sv%s/tokens", opt_swift_auth_url,
             opt_swift_auth_version);
  ...
  if (opt_swift_storage_url != NULL) {
    snprintf(info.url, sizeof(info.url), "%s", opt_swift_storage_url);
  }
  ...
  if (opt_mode == MODE_PUT) {
    ...
    if (swift_backup_exists(&info, opt_swift_container, opt_name)) {
      fprintf(stderr,
              "error: backup named '%s' "
              "already exists!\n",
              opt_name);
      return (EXIT_FAILURE);
    }

    if (swift_upload_parts(&info, opt_swift_container, opt_name) != 0) {
      fprintf(stderr, "error: upload failed\n");
      return (EXIT_FAILURE);
    }

  } else if (opt_mode == MODE_GET) {
    if (swift_download(&info, opt_swift_container, opt_name) != CURLE_OK) {
      fprintf(stderr, "error: download failed\n");
      return (EXIT_FAILURE);
    }

  } else if (opt_mode == MODE_DELETE) {
    if (swift_delete(&info, opt_swift_container, opt_name) != CURLE_OK) {
