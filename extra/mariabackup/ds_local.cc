...
static ds_ctxt_t *local_init(const char *root) {
  ...
  if (my_mkdir(root, 0777, MYF(0)) < 0 && my_errno != EEXIST &&
    ...
    my_error(EE_CANT_MKDIR, MYF(ME_BELL), root, my_errno, errbuf, my_errno);
    return NULL;
  }
...
static ds_file_t *local_open(ds_ctxt_t *ctxt, const char *path,
  ...
  if (my_mkdir(dirpath, 0777, MYF(0)) < 0 && my_errno != EEXIST) {
    char errbuf[MYSYS_STRERROR_SIZE];
    my_strerror(errbuf, sizeof(errbuf), my_errno);
    my_error(EE_CANT_MKDIR, MYF(ME_BELL), dirpath, my_errno, errbuf);
    return NULL;
  }
  ...
  local_file->is_ibd =
      (path_len > 5) && !strcmp(fullpath + path_len - 5, ".ibd");
...
*/
static void init_ibd_data(ds_local_file_t *local_file, const uchar *buf,
                          size_t len) {
  ...
  local_file->pagesize =
      ssize == 0 ? UNIV_PAGE_SIZE_ORIG : ((UNIV_ZIP_SIZE_MIN >> 1) << ssize);
