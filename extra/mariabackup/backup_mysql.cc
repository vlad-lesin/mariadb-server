...
MYSQL *xb_mysql_connect() {
  ...
  if (connection == NULL) {
    msg("Failed to init MySQL struct: %s.", mysql_error(connection));
    return (NULL);
  }

  if (!opt_secure_auth) {
    mysql_options(connection, MYSQL_SECURE_AUTH, (char *)&opt_secure_auth);
  }
...
MYSQL_RES *xb_mysql_query(MYSQL *connection, const char *query, bool use_result,
  ...
  if (mysql_query(connection, query)) {
    if (die_on_error) {
      ...
      msg("Error: failed to execute query %s: %s", query,
  ...
  if (mysql_field_count(connection) > 0) {
    if ((mysql_result = mysql_store_result(connection)) == NULL) {
      die("failed to fetch query result %s: %s", query,
          mysql_error(connection));
    }
  ...
  if (vertical_result) {
    while ((row = mysql_fetch_row(mysql_result))) {
      ...
      for (var = vars; var->name; var++) {
        if (strcmp(var->name, name) == 0 && value != NULL) {
...
static char *read_mysql_one_value(MYSQL *connection, const char *query) {
  MYSQL_RES *mysql_result;
  MYSQL_ROW row;
  char *result = NULL;
...
static bool check_server_version(unsigned long version_number,
  ...
  version_supported = version_supported || (mysql51 && innodb_version != NULL);
  version_supported =
      version_supported || (version_number > 50500 && version_number < 50700);
  version_supported = version_supported || ((version_number > 100000) &&
                                            server_flavor == FLAVOR_MARIADB);
...
static bool select_incremental_lsn_from_history(lsn_t *incremental_lsn) {
  ...
  if (opt_incremental_history_name) {
    mysql_real_escape_string(
        mysql_connection, buf, opt_incremental_history_name,
  ...
  if (opt_incremental_history_uuid) {
    mysql_real_escape_string(
        mysql_connection, buf, opt_incremental_history_uuid,
  ...
  if (row) {
    *incremental_lsn = strtoull(row[0], NULL, 10);
    msg("Found and using lsn: " LSN_PF " for %s %s", *incremental_lsn,
...
static bool have_queries_to_wait_for(MYSQL *connection, uint threshold) {
  MYSQL_RES *result = xb_mysql_query(connection, "SHOW FULL PROCESSLIST", true);
  ...
  while (MYSQL_ROW row = mysql_fetch_row(result)) {
    ...
    if (info != NULL && duration >= (int)threshold &&
      ...
      msg("Waiting for query %s (duration %d sec): %s", id, duration, info);
      have_to_wait = true;
      break;
    }
  }
...
static void kill_long_queries(MYSQL *connection, time_t timeout) {
  char kill_stmt[100];
  ...
  while (MYSQL_ROW row = mysql_fetch_row(result)) {
    ...
    if (info != NULL && (time_t)duration >= timeout &&
        ((all_queries && is_query(info)) || is_select_query(info))) {
      msg("Killing query %s (duration %d sec): %s", id, (int)duration, info);
...
static os_thread_ret_t DECLARE_THREAD(kill_query_thread)(
    /*===============*/
...
/*********************************************************************/ /**
 ...
 @returns true if lock acquired */
bool lock_binlog_maybe(MYSQL *connection) {
...
void unlock_all(MYSQL *connection) {
  if (opt_debug_sleep_before_unlock) {
    msg("Debug sleep for %u seconds", opt_debug_sleep_before_unlock);
...
bool write_slave_info(MYSQL *connection) {
  ...
  mysql_variable variables[] = {{"gtid_slave_pos", &gtid_slave_pos},
                                {NULL, NULL}};
  ...
  if (gtid_executed != NULL && *gtid_executed) {
    ...
    ut_a(asprintf(&mysql_slave_position, "master host '%s', purge list '%s'",
                  master, gtid_executed) != -1);
  } else if (gtid_slave_pos && *gtid_slave_pos) {
    /* MariaDB >= 10.0 with GTID enabled */
    result =
        backup_file_printf(XTRABACKUP_SLAVE_INFO,
                           "SET GLOBAL gtid_slave_pos = '%s';\n"
                           "CHANGE MASTER TO master_use_gtid = slave_pos\n",
                           gtid_slave_pos);
    ut_a(asprintf(&mysql_slave_position, "master host '%s', gtid_slave_pos %s",
    ...
    ut_a(asprintf(&mysql_slave_position,
                  "master host '%s', filename '%s', position '%s'", master,
                  filename, position) != -1);
  }
...
bool write_binlog_info(MYSQL *connection) {
  ...
  mysql_variable status[] = {{"File", &filename},
                             {"Position", &position},
                             {"Executed_Gtid_Set", &gtid_executed},
                             {NULL, NULL}};

  mysql_variable vars[] = {{"gtid_mode", &gtid_mode},
  ...
  if (mariadb_gtid || mysql_gtid) {
    ...
    result = backup_file_printf(XTRABACKUP_BINLOG_INFO, "%s\t%s\t%s\n",
                                filename, position, gtid);
  } else {
    ut_a(asprintf(&mysql_binlog_position, "filename '%s', position '%s'",
                  filename, position) != -1);
...
struct escape_and_quote {
  escape_and_quote(MYSQL *mysql, const char *str) : mysql(mysql), str(str) {}
  MYSQL *const mysql;
  const char *const str;
};

static std::ostream &operator<<(std::ostream &s, const escape_and_quote &eq) {
  if (!eq.str) return s << "NULL";
  s << '\'';
...
/*********************************************************************/ /**
 ...
 backup. */
bool write_xtrabackup_info(MYSQL *connection, const char *filename,
                           bool history) {
  char *uuid = NULL;
  ...
  strftime(buf_start_time, sizeof(buf_start_time), "%Y-%m-%d %H:%M:%S", &tm);
  history_end_time = time(NULL);
  localtime_r(&history_end_time, &tm);
  strftime(buf_end_time, sizeof(buf_end_time), "%Y-%m-%d %H:%M:%S", &tm);
  bool is_partial = (xtrabackup_tables || xtrabackup_tables_file ||
                     xtrabackup_databases || xtrabackup_databases_file ||
                     xtrabackup_tables_exclude || xtrabackup_databases_exclude);
  ...
  backup_file_printf(
      ...
      mysql_binlog_position ? mysql_binlog_position : "", /* binlog_pos */
...
#define ESCAPE_BOOL(expr) ((expr) ? "'Y'" : "'N'")
  ...
  oss << "insert into PERCONA_SCHEMA.xtrabackup_history("
      ...
      << "values(" << escape_and_quote(connection, uuid) << ","
      ...
      << "from_unixtime(" << history_end_time << ")," << history_lock_time
      << "," << escape_and_quote(connection, mysql_binlog_position) << ","
      << incremental_lsn << "," << metadata_to_lsn << ","
      << ESCAPE_BOOL(is_partial) << "," << ESCAPE_BOOL(xtrabackup_incremental)
      << ","
...
bool write_backup_config_file() {
  int rc = backup_file_printf(
      ...
      make_local_paths(innobase_data_file_path).c_str(), srv_n_log_files,
      srv_log_file_size, srv_page_size, srv_undo_dir, srv_undo_tablespaces,
      page_zip_level,
      innobase_buffer_pool_filename ? "innodb_buffer_pool_filename=" : "",
...
void mdl_lock_init() {
  ...
  const char *query =
      "SELECT NAME, SPACE FROM INFORMATION_SCHEMA.INNODB_SYS_TABLES WHERE NAME "
      "LIKE '%%/%%'";
...
void mdl_lock_table(ulint space_id) {
  ...
  DBUG_EXECUTE_IF(
      ...
      if (full_table_name == "`test`.`t1`") xb_mysql_query(
          mysql_connection, "RENAME TABLE test.t1 to test.t2", false, true););
  ...
  if (mysql_query(mdl_con, lock_query.str().c_str())) {
    msg("Warning : locking MDL failed for space id %zu, name %s", space_id,
