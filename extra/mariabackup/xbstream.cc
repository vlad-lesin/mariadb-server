...
static int stream_one_file(File file, xb_wstream_file_t *xbfile) {
  ...
  while ((bytes = (ssize_t)my_read(file, buf, XBSTREAM_BUFFER_SIZE,
    ...
    if (xb_stream_write_data(xbfile, buf, bytes)) {
      msg("%s: xb_stream_write_data() failed.", my_progname);
      my_free(buf);
      return 1;
    }
...
static int mode_create(int argc, char **argv) {
  ...
  for (i = 0; i < argc; i++) {
    ...
    if (!MY_S_ISREG(mystat.st_mode)) {
      msg("%s: %s is not a regular file, exiting.", my_progname, filepath);
      goto err;
    }
...
static uchar *get_file_entry_key(file_entry_t *entry, size_t *length,
...
static void *extract_worker_thread_func(void *arg) {
  ...
  while (1) {
    ...
    entry = (file_entry_t *)my_hash_search(ctxt->filehash, (uchar *)chunk.path,
                                           chunk.pathlen);
    ...
    if (entry == NULL) {
      ...
      if (my_hash_insert(ctxt->filehash, (uchar *)entry)) {
        msg("%s: my_hash_insert() failed.", my_progname);
        pthread_mutex_unlock(ctxt->mutex);
        break;
      }
    }
...
static int mode_extract(int n_threads, int argc __attribute__((unused)),
  ...
  if (my_hash_init(&filehash, &my_charset_bin, START_FILE_HASH_SIZE, 0, 0,
                   (my_hash_get_key)get_file_entry_key,
