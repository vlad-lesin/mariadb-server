 ...
 */
enum { MODIFIED_PAGE_BLOCK_SIZE = 4096 };
    ...
    const void *p2) /*!<in: 2nd key to compare */
{
  ...
  if (k1_space == k2_space) {
    ulint k1_start_page = mach_read_from_4(k1 + MODIFIED_PAGE_1ST_PAGE_ID);
    ulint k2_start_page = mach_read_from_4(k2 + MODIFIED_PAGE_1ST_PAGE_ID);
                                           ...
                                           checksum OK */
{
  ...
  success = os_file_read(IORequestRead, bitmap_file->file, page,
 ...
 */
static ibool log_online_setup_bitmap_file_range(
    /*===============================*/
    ...
    lsn_t range_end)                              /*!<in: end LSN */
{
  ...
  if (UNIV_UNLIKELY(!bitmap_dir)) {
    msg("InnoDB: Error: failed to open bitmap directory \'%s\'", srv_data_home);
    return FALSE;
  }
  ...
  while (!os_file_readdir_next_file(srv_data_home, bitmap_dir,
    ...
    if (!log_online_is_bitmap_file(&bitmap_dir_file_info, &file_seq_num,
                                   &file_start_lsn) ||
  ...
  bitmap_files->files = static_cast<log_online_bitmap_file_range_t::files_t *>(
  ...
  while (!os_file_readdir_next_file(srv_data_home, bitmap_dir,
    ...
    if (file_seq_num > bitmap_files->files[array_pos].seq_num) {
      ...
      strncpy(bitmap_files->files[array_pos].name, bitmap_dir_file_info.name,
              FN_REFLEN);
  ...
  for (size_t i = 1; i < bitmap_files->count; i++) {
    ...
    ut_ad(bitmap_files->files[i].seq_num > bitmap_files->files[i - 1].seq_num);
                                           ...
                                           file */
{
  ...
  bitmap_file->file = os_file_create_simple_no_error_handling(
      0, bitmap_file->name, OS_FILE_OPEN, OS_FILE_READ_ONLY, true, &success);
  if (UNIV_UNLIKELY(!success)) {
...
static void xb_msg_missing_lsn_data(
    /*====================*/
    lsn_t missing_interval_start, /*!<in: interval start */
    lsn_t missing_interval_end)   /*!<in: interval end */
{
  msg("mariabackup: warning: changed page data missing for LSNs between " LSN_PF
      " and " LSN_PF,
...
/*=====================*/
{
  ...
  result = rbt_create(MODIFIED_PAGE_BLOCK_SIZE, log_online_compare_bmp_keys);
  ...
  if (UNIV_UNLIKELY(bitmap_files.files[bmp_i].start_lsn > bmp_start_lsn)) {
  ...
  while (last_page_ok &&
         (current_page_end_lsn < bmp_end_lsn ||
          (current_page_end_lsn == bmp_end_lsn && !last_page_in_run))) {
    ...
    if (!rbt_search(result, &tree_search_pos, page)) {
      ...
      byte *existing_page = rbt_value(byte, tree_search_pos.last);
      bitmap_word_t *bmp_word_1 =
          (bitmap_word_t *)(existing_page + MODIFIED_PAGE_BLOCK_BITMAP);
      bitmap_word_t *bmp_end =
          (bitmap_word_t *)(existing_page + MODIFIED_PAGE_BLOCK_UNUSED_2);
      bitmap_word_t *bmp_word_2 =
          (bitmap_word_t *)(page + MODIFIED_PAGE_BLOCK_BITMAP);
    ...
    current_page_end_lsn = mach_read_from_8(page + MODIFIED_PAGE_END_LSN);
    last_page_in_run = mach_read_from_4(page + MODIFIED_PAGE_IS_LAST_BLOCK);
  }
    ...
    xb_page_bitmap_range *bitmap_range) /*!<in/out: the bitmap range */
{
  ...
  new_space_id =
      mach_read_from_4(bitmap_range->bitmap_page + MODIFIED_PAGE_SPACE_ID);
  ...
  new_1st_page_id =
      mach_read_from_4(bitmap_range->bitmap_page + MODIFIED_PAGE_1ST_PAGE_ID);
...
static const ib_rbt_node_t *rbt_lower_bound(const ib_rbt_t *tree,
                                            const void *key) {
  ut_ad(!tree->cmp_arg);
  const ib_rbt_node_t *ge = NULL;

  for (const ib_rbt_node_t *node = tree->root->left; node != tree->nil;) {
  ...
  do {
    while (bitmap_range->bit_i < MODIFIED_PAGE_BLOCK_ID_COUNT) {
      while (is_bit_set(bitmap_range) != bit_value &&
             (bitmap_range->bit_i < MODIFIED_PAGE_BLOCK_ID_COUNT)) {
        bitmap_range->current_page_id++;
        bitmap_range->bit_i++;
      }

      if (bitmap_range->bit_i < MODIFIED_PAGE_BLOCK_ID_COUNT) {
    ...
    bitmap_range->bitmap_node =
        rbt_next(bitmap_range->bitmap, bitmap_range->bitmap_node);
