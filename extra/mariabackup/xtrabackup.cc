...
static fil_node_t *datafiles_iter_next(datafiles_iter_t *it) {
  fil_node_t *new_node;

  pthread_mutex_lock(&it->mutex);

  if (it->node == NULL) {
    if (it->started) goto end;
    it->started = TRUE;
  } else {
    it->node = UT_LIST_GET_NEXT(chain, it->node);
    if (it->node != NULL) goto end;
  }

  it->space = (it->space == NULL) ? UT_LIST_GET_FIRST(fil_system.space_list)
                                  : UT_LIST_GET_NEXT(space_list, it->space);

  while (it->space != NULL && (it->space->purpose != FIL_TYPE_TABLESPACE ||
                               UT_LIST_GET_LEN(it->space->chain) == 0))
    it->space = UT_LIST_GET_NEXT(space_list, it->space);
  if (it->space == NULL) goto end;
...
static void datafiles_iter_free(datafiles_iter_t *it) {
...
extern "C" os_thread_ret_t DECLARE_THREAD(dbug_execute_in_new_connection)(
    void *arg) {
  mysql_thread_init();
  ...
  if (par->expect_err != err) {
    msg("FATAL: dbug_execute_in_new_connection : mysql_query '%s' returns %d, "
        "instead of expected %d",
        par->query, err, par->expect_err);
    _exit(1);
  }
  if (err && par->expect_errno && par->expect_errno != err_no) {
    msg("FATAL: dbug_execute_in_new_connection: mysql_query '%s' returns "
        "mysql_errno %d, instead of expected %d",
...
*/
static os_event_t dbug_start_query_thread(const char *query,
...

{
  ...
  if (!wait_state) return par->done_event;
  ...
  for (;;) {
    ...
    msg("Waiting for query '%s' on connection %lu to "
        " reach state '%s'",
        query, mysql_thread_id(par->con), wait_state);
    my_sleep(1000);
  }
end:
...
static void backup_file_op(ulint space_id, const byte *flags, const byte *name,
                           ulint len, const byte *new_name, ulint new_len) {
  ...
  if (flags) {
    ...
    msg("DDL tracking :  create %zu \"%.*s\": %x", space_id, int(len), name,
        mach_read_from_4(flags));
  } else if (new_name) {
    ddl_tracker.id_to_name[space_id] = filename_to_spacename(new_name, new_len);
    msg("DDL tracking : rename %zu \"%.*s\",\"%.*s\"", space_id, int(len), name,
        int(new_len), new_name);
  } else {
...
static void backup_file_op_fail(ulint space_id, const byte *flags,
  ...
  if (flags) {
    ...
    fail = !check_if_skip_table(spacename.c_str()) ||
           !check_if_skip_table(new_spacename.c_str());
  } else {
    std::string spacename = filename_to_spacename(name, len);
    fail = !check_if_skip_table(spacename.c_str());
    msg("DDL tracking : delete %zu \"%.*s\"", space_id, int(len), name);
...
struct my_option xb_server_options[] = {
    ...
    {"extended_validation", OPT_XTRA_EXTENDED_VALIDATION,
     ...
     (G_PTR *)&opt_extended_validation, (G_PTR *)&opt_extended_validation, 0,
    ...
    {"encrypted_backup", OPT_XTRA_ENCRYPTED_BACKUP,
     ...
     (G_PTR *)&opt_encrypted_backup, (G_PTR *)&opt_encrypted_backup, 0,
    ...
    {"log_bin", OPT_LOG, "Base name for the log sequence", &opt_log_bin,
     &opt_log_bin, 0, GET_STR, OPT_ARG, 0, 0, 0, 0, 0, 0},
    ...
    {"innodb_adaptive_hash_index", OPT_INNODB_ADAPTIVE_HASH_INDEX,
     ...
     &btr_search_enabled, &btr_search_enabled, 0, GET_BOOL, NO_ARG, 1, 0, 0, 0,
     0, 0},
    ...
    {"innodb_flush_method", OPT_INNODB_FLUSH_METHOD,
     ...
     &srv_file_flush_method, &innodb_flush_method_typelib, GET_ENUM,
     REQUIRED_ARG, IF_WIN(SRV_ALL_O_DIRECT_FSYNC, SRV_FSYNC), 0, 0, 0, 0, 0},

    {"innodb_log_buffer_size", OPT_INNODB_LOG_BUFFER_SIZE,
     "The size of the buffer which InnoDB uses to write log to the log files "
     "on disk.",
     (G_PTR *)&srv_log_buffer_size, (G_PTR *)&srv_log_buffer_size, 0, GET_ULONG,
     REQUIRED_ARG, 1024 * 1024L, 256 * 1024L, LONG_MAX, 0, 1024, 0},
    {"innodb_log_file_size", OPT_INNODB_LOG_FILE_SIZE,
     "Ignored for mysqld option compatibility", (G_PTR *)&srv_log_file_size,
     (G_PTR *)&srv_log_file_size, 0, GET_ULL, REQUIRED_ARG, 48 << 20, 1 << 20,
     512ULL << 30, 0, UNIV_PAGE_SIZE_MAX, 0},
    {"innodb_log_files_in_group", OPT_INNODB_LOG_FILES_IN_GROUP,
     "Ignored for mysqld option compatibility", &srv_n_log_files,
     &srv_n_log_files, 0, GET_LONG, REQUIRED_ARG, 1, 1, 100, 0, 1, 0},
    ...
    {"dbug", '#', "Built in DBUG debugger.", &dbug_option, &dbug_option, 0,
     GET_STR, OPT_ARG, 0, 0, 0, 0, 0, 0},
#endif
#ifndef __WIN__
    ...
    {"innodb_undo_directory", OPT_INNODB_UNDO_DIRECTORY,
     ...
     &srv_undo_dir, &srv_undo_dir, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},

    {"innodb_undo_tablespaces", OPT_INNODB_UNDO_TABLESPACES,
     "Number of undo tablespaces to use.", (G_PTR *)&srv_undo_tablespaces,
     (G_PTR *)&srv_undo_tablespaces, 0, GET_ULONG, REQUIRED_ARG, 0, 0, 126, 0,
     1, 0},

    {"innodb_compression_level", OPT_INNODB_COMPRESSION_LEVEL,
     "Compression level used for zlib compression.", (G_PTR *)&page_zip_level,
     (G_PTR *)&page_zip_level, 0, GET_UINT, REQUIRED_ARG, 6, 0, 9, 0, 0, 0},
    ...
    {"plugin-dir", OPT_PLUGIN_DIR,
     "Server plugin directory. Used to load encryption plugin during 'prepare' "
     "phase."
     "Has no effect in the 'backup' phase (plugin directory during backup is "
     "the same as server's)",
     &xb_plugin_dir, &xb_plugin_dir, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0,
     0},

    {"innodb-log-checksums", OPT_INNODB_LOG_CHECKSUMS,
     "Whether to require checksums for InnoDB redo log blocks",
     &innodb_log_checksums, &innodb_log_checksums, 0, GET_BOOL, REQUIRED_ARG, 1,
     0, 0, 0, 0, 0},
    ...
    {"rocksdb-datadir", OPT_ROCKSDB_DATADIR,
     "RocksDB data directory."
     "This option is only  used with --copy-back or --move-back option",
     &xb_rocksdb_datadir, &xb_rocksdb_datadir, 0, GET_STR, REQUIRED_ARG, 0, 0,
     0, 0, 0, 0},

    {"rocksdb-backup", OPT_BACKUP_ROCKSDB,
     "Backup rocksdb data, if rocksdb plugin is installed."
     "Used only with --backup option. Can be useful for partial backups, to "
     "exclude all rocksdb data",
     &xb_backup_rocksdb, &xb_backup_rocksdb, 0, GET_BOOL, NO_ARG, 1, 0, 0, 0, 0,
     0},

    {"check-privileges", OPT_XTRA_CHECK_PRIVILEGES,
     "Check database user "
     "privileges fro the backup user",
     &opt_check_privileges, &opt_check_privileges, 0, GET_BOOL, NO_ARG, 1, 0, 0,
     0, 0, 0},
...
static int prepare_export() {
  ...
  if (create_bootstrap_file()) return -1;
...
static const char *xb_client_default_groups[] = {"xtrabackup",
                                                 "mariabackup",
                                                 "client",
                                                 "client-server",
                                                 "client-mariadb",
...
static const char *xb_server_default_groups[] = {"xtrabackup",
                                                 "mariabackup",
                                                 "mysqld",
                                                 "server",
                                                 MYSQL_BASE_VERSION,
...
You can download full text of the license on http://www.gnu.org/licenses/gpl-2.0.txt\n");
  ...
  printf(
      "Usage: %s [--defaults-file=#] [--backup | --prepare | --copy-back | "
      "--move-back] [OPTIONS]\n",
      my_progname);
...
my_bool xb_get_one_option(int optid,
  ...
  switch (optid) {
    ...
    case OPT_PROTOCOL:
      if (argument) {
        if ((opt_protocol = find_type_with_warning(
                 argument, &sql_protocol_typelib, opt->name)) <= 0) {
...
static bool innodb_init_param() {
  ...
  srv_data_home =
      (xtrabackup_backup && innobase_data_home_dir ? innobase_data_home_dir
                                                   : default_path);
...
static my_bool xtrabackup_read_metadata(char *filename) {
  FILE *fp;
  my_bool r = TRUE;
...
@return TRUE on success, FALSE on failure. */
static my_bool xtrabackup_stream_metadata(ds_ctxt_t *ds_ctxt) {
  char buf[1024];
  size_t len;
  ds_file_t *stream;
  MY_STAT mystat;
  ...
  if (stream == NULL) {
    msg("Error: cannot open output stream for %s",
...
@return TRUE on success, FALSE on failure. */
static my_bool xtrabackup_write_metadata(const char *filepath) {
  char buf[1024];
  size_t len;
  FILE *fp;
...
@return TRUE on success, FALSE on failure. */
static my_bool xb_read_delta_metadata(const char *filepath,
...
static my_bool regex_list_check_match(const regex_list_t &list,
                                      const char *name) {
...
static my_bool find_filter_in_hashtable(const char *name, hash_table_t *table,
...
static my_bool check_if_table_matches_filters(const char *name,
...
my_bool check_if_skip_database_by_path(
...
static my_bool xtrabackup_copy_datafile(fil_node_t *node, uint thread_n,
  ...
  pthread_mutex_lock(&backup_mutex);
  was_dropped =
      (ddl_tracker.drops.find(node->space->id) != ddl_tracker.drops.end());
  pthread_mutex_unlock(&backup_mutex);
  if (was_dropped) {
    if (node->is_open()) {
...
skip:
  ...
  msg(thread_n,
      "Warning: We assume the  table was dropped during xtrabackup execution "
      "and ignore the tablespace %s",
      node_name);
  return (FALSE);
}
...
static lsn_t xtrabackup_copy_log(lsn_t start_lsn, lsn_t end_lsn, bool last) {
  ...
  for (ulint scanned_checkpoint = 0; scanned_lsn < end_lsn;
    ...
    more_data = recv_sys_add_to_parsing_buf(log_block, scanned_lsn + data_len);
  ...
  end_lsn = last ? ut_uint64_align_up(scanned_lsn, OS_FILE_LOG_BLOCK_SIZE)
...
static bool xtrabackup_copy_logfile(bool last = false) {
  ...
  start_lsn =
      ut_uint64_align_down(log_copy_scanned_lsn, OS_FILE_LOG_BLOCK_SIZE);
  do {
    ...
    for (int retries = 0; retries < 100; retries++) {
      if (log_sys.log.read_log_seg(&lsn, end_lsn) || lsn != start_lsn) {
        break;
      }
...
static os_thread_ret_t DECLARE_THREAD(log_copying_thread)(void *) {
  ...
  */
  ...
  for (;;) {
    ...
    log_mutex_enter();
    bool completed = metadata_to_lsn && metadata_to_lsn <= log_copy_scanned_lsn;
    log_mutex_exit();
    if (completed) {
      break;
    }
...
2. Set environment variable my_event_name_$key SQL statement you want to execute
   ...
   In mtr , you can set environment via 'let' statement (do not use $ as the
first char for the variable)
...
static os_thread_ret_t DECLARE_THREAD(data_copy_thread_func)(
    /*==================*/
...
static void xb_fil_io_init() {
  fil_system.create(srv_file_per_table ? 50000 : 5000);
}

static Datafile *xb_new_datafile(const char *name, bool is_remote) {
  if (is_remote) {
...
static void xb_load_single_table_tablespace(const char *dirname,
  ...
  if (err == DB_SUCCESS && file->space_id() != SRV_TMP_SPACE_ID) {
    ...
    space = fil_space_create(name, file->space_id(), file->flags(),
                             FIL_TYPE_TABLESPACE, NULL /* TODO: crypt_data */);

    ut_a(space != NULL);

    space->add(file->filepath(), OS_FILE_CLOSED, ulint(n_pages), false, false);
    /* by opening the tablespace we forcing node and space objects
    in the cache to be populated with fields from space header */
    space->open();

    if (srv_operation == SRV_OPERATION_RESTORE_DELTA || xb_close_files) {
      space->close();
    }
  }
  ...
  if (err != DB_SUCCESS && xtrabackup_backup && !is_empty_file) {
    die("Failed to not validate first page of the file %s, error %d", name,
        (int)err);
  }
}
...
static dberr_t enumerate_ibd_files(process_single_tablespace_func_t callback) {
  ...
  ret =
      fil_file_readdir_next_file(&err, fil_path_to_mysql_datadir, dir, &dbinfo);
  while (ret == 0) {
    ...
    snprintf(dbpath, dbpath_len, "%s/%s", fil_path_to_mysql_datadir,
             dbinfo.name);
    ...
    if (dbdir != NULL) {
      ...
      for (ret = fil_file_readdir_next_file(&err, dbpath, dbdir, &fileinfo);
           ret == 0;
        ...
        if (strlen(fileinfo.name) > 4) {
          bool is_isl = false;
          if (ends_with(fileinfo.name, ".ibd") ||
              ((is_isl = ends_with(fileinfo.name, ".isl"))))
  ...
  if (0 != os_file_closedir(dir)) {
    fprintf(stderr, "InnoDB: Error: could not close MySQL datadir\n");
...
/** Assign srv_undo_space_id_start variable if there are undo tablespace
present. Read the TRX_SYS page from ibdata1 file and get the minimum space id
from the first slot rollback segments of TRX_SYS_PAGE_NO.
...
static dberr_t xb_assign_undo_space_start() {
  ...
  fsp_flags = mach_read_from_4(page + FSP_HEADER_OFFSET + FSP_SPACE_FLAGS);
retry:
  if (!os_file_read(IORequestRead, file, page,
                    TRX_SYS_PAGE_NO << srv_page_size_shift, srv_page_size)) {
  ...
  ut_ad(mach_read_from_4(TRX_SYS + TRX_SYS_RSEGS + TRX_SYS_RSEG_SLOT_SIZE +
                         TRX_SYS_RSEG_PAGE_NO + page) != FIL_NULL);

  space = mach_read_ulint(TRX_SYS + TRX_SYS_RSEGS + TRX_SYS_RSEG_SLOT_SIZE +
...
opening data files.
@returns DB_SUCCESS or error code.*/
static dberr_t xb_data_files_init() {
  xb_fil_io_init();
    ...
    regex_list_t *list)        /*! in: list to put new regex to */
{
  ...
  if (ret != 0) {
    ...
    msg("mariabackup: error: %s regcomp(%s): %s", error_context, regex, errbuf);
    exit(EXIT_FAILURE);
  }
...
static void open_or_create_log_file(
    /*====================*/
    fil_space_t *space, ulint i) /*!< in: log file number in group */
{
  char name[10000];
  ulint dirnamelen;
  ...
  space->add(name, OS_FILE_CLOSED,
             ulint(srv_log_file_size >> srv_page_size_shift), false, false);
}
...
static bool xtrabackup_backup_func() {
  ...
  msg("open files limit requested %u, set to %u", (uint)xb_open_files_limit,
  ...
  fil_space_t *space = fil_space_create(
      "innodb_redo_log", SRV_LOG_SPACE_FIRST_ID, 0, FIL_TYPE_LOG, NULL);
  ...
  reread_log_header:
    ...
    log_block_set_checksum(log_hdr, log_block_calc_checksum_crc32(log_hdr));
    ...
    if (xtrabackup_copy_logfile()) goto fail_before_log_copying_thread_start;
    ...
    if (opt_lock_ddl_per_table) {
      ...
      DBUG_EXECUTE_IF("check_mdl_lock_works",
                      dbug_alter_thread_done = dbug_start_query_thread(
    ...
    data_threads = (data_thread_ctxt_t *)malloc(sizeof(data_thread_ctxt_t) *
                                                xtrabackup_parallel);
...
- Tables that were deleted during backup, are marked as deleted
  ...
  It is the responsibility of the prepare phase to deal with .new, .ren, and
.del files.
*/
void backup_fix_ddl(void) {
  ...
  for (space_id_to_name_t::iterator iter = ddl_tracker.tables_in_backup.begin();
       iter != ddl_tracker.tables_in_backup.end(); iter++) {
  ...
  for (space_id_to_name_t::iterator iter = ddl_tracker.id_to_name.begin();
       iter != ddl_tracker.id_to_name.end(); iter++) {
    ulint id = iter->first;
    std::string name = iter->second;

    if (ddl_tracker.tables_in_backup.find(id) !=
        ddl_tracker.tables_in_backup.end()) {
  ...
  for (std::set<std::string>::iterator iter = new_tables.begin();
    ...
    if (check_if_skip_table(space_name)) continue;
  ...
  while (fil_node_t *node = datafiles_iter_next(it)) {
    ...
    if (!fil_is_user_tablespace_id(space->id)) continue;
    std::string dest_name(node->space->name);
    dest_name.append(".new");
    xtrabackup_copy_datafile(
        node, 0,
        dest_name.c_str() /*, do_full_copy ? ULONGLONG_MAX:UNIV_PAGE_SIZE */);
  }
    ...
    pfs_os_file_t *file) /*!<out: file handle */
{
  ...
  *file = os_file_create_simple_no_error_handling(
  ...
  if (!zip_size) {
    ...
    ret = os_file_write(IORequestWrite, path, *file, page, 0, srv_page_size);
  } else {
    ...
    ret =
        os_file_write(IORequestWrite, path, *file, page_zip.data, 0, zip_size);
  }

  free(buf);

  if (!ret) {
    msg("mariabackup: could not write the first page to %s", path);
    os_file_close(*file);
    os_file_delete(0, path);
    return ret;
  }
...
static fil_space_t *fil_space_get_by_name(const char *name) {
  ...
  for (fil_space_t *space = UT_LIST_GET_FIRST(fil_system.space_list);
       space != NULL; space = UT_LIST_GET_NEXT(space_list, space))
    ...
    bool *success)        /* out: indicates error. true = success */
{
  ...
  ut_a(dbname != NULL || !fil_is_user_tablespace_id(info.space_id) ||
       info.space_id == ULINT_UNDEFINED);
  ...
  *success = false;
  ...
  found:
    ...
    file = os_file_create_simple_no_error_handling(
        0, real_name, OS_FILE_OPEN, OS_FILE_READ_WRITE, false, success);
  ...
  table = static_cast<xb_filter_entry_t *>(
      malloc(sizeof(xb_filter_entry_t) + strlen(dest_space_name) + 1));
  ...
  if (fil_space != NULL) {
    if (fil_space->id == info.space_id || info.space_id == ULINT_UNDEFINED) {
      /* we found matching space */
      goto found;
    } else {
      char tmpname[FN_REFLEN];

      snprintf(tmpname, FN_REFLEN, "%s/xtrabackup_tmp_#" ULINTPF, dbname,
               fil_space->id);

      msg("mariabackup: Renaming %s to %s.ibd", fil_space->name, tmpname);

      if (fil_space->rename(tmpname, NULL, false) != DB_SUCCESS) {
        msg("mariabackup: Cannot rename %s to %s", fil_space->name, tmpname);
        goto exit;
      }
    }
  }
  ...
  if (fil_space != NULL) {
    ...
    msg("mariabackup: Renaming %s to %s", fil_space->name, dest_space_name);

    if (fil_space->rename(tmpname, NULL, false) != DB_SUCCESS) {
      msg("mariabackup: Cannot rename %s to %s", fil_space->name,
          dest_space_name);
      goto exit;
    }
  ...
  const ulint flags =
      info.zip_size
          ? get_bit_shift(info.page_size >> (UNIV_ZIP_SIZE_SHIFT_MIN - 1))
  ...
  if (fil_space_create(dest_space_name, info.space_id, flags,
    ...
    *success = xb_space_create_file(real_name, info.space_id, flags, &file);
  } else {
...
static ibool xtrabackup_apply_delta(
  ...
  if (dbname) {
    snprintf(src_path, sizeof(src_path), "%s/%s/%s", dirname, dbname, filename);
    snprintf(dst_path, sizeof(dst_path), "%s/%s/%s", xtrabackup_real_target_dir,
             dbname, filename);
  } else {
    snprintf(src_path, sizeof(src_path), "%s/%s", dirname, filename);
  ...
  msg("page size for %s is %zu bytes", src_path, page_size);
  ...
  src_file = os_file_create_simple_no_error_handling(
      0, src_path, OS_FILE_OPEN, OS_FILE_READ_WRITE, false, &success);
  if (!success) {
  ...
  dst_file = xb_delta_open_matching_space(dbname, space_name, info, dst_path,
  ...
  incremental_buffer_base =
      static_cast<byte *>(malloc((page_size / 4 + 1) * page_size));
  incremental_buffer =
      static_cast<byte *>(ut_align(incremental_buffer_base, page_size));
  ...
  while (!last_buffer) {
    ...
    success = os_file_read(IORequestRead, src_file, incremental_buffer, offset,
                           page_size);
    if (!success) {
      goto error;
    }
    ...
    for (page_in_buffer = 1; page_in_buffer < page_size / 4; page_in_buffer++) {
      ...
      if (off == 0) {
        ...
        os_offset_t n_pages =
            mach_read_from_4(buf + FSP_HEADER_OFFSET + FSP_SIZE);
        if (mach_read_from_4(buf + FIL_PAGE_SPACE_ID)) {
          if (!os_file_set_size(dst_path, dst_file, n_pages * page_size))
            goto error;
        } else if (fil_space_t *space = fil_system.sys_space) {
          ...
          bool fail = !strcmp(n->name, dst_path) &&
                      !fil_space_extend(space, (ulint)n_pages);
          if (fail) goto error;
        }
      }

      success = os_file_write(IORequestWrite, dst_path, dst_file, buf, off,
                              page_size);
      if (!success) {
        goto error;
      }
  ...
  if (dst_file != OS_FILE_CLOSED) os_file_close(dst_file);
  return TRUE;

error:
  free(incremental_buffer_base);
  if (src_file != OS_FILE_CLOSED) os_file_close(src_file);
  if (dst_file != OS_FILE_CLOSED) os_file_close(dst_file);
...
static ibool prepare_handle_new_files(
    ...
    void *) {
  std::string src_path =
      std::string(data_home_dir) + '/' + std::string(db_name) + '/' + file_name;
...
static ibool xb_process_datadir(
  ...
  if (dbdir != NULL) {
    ...
    while (ret == 0) {
      ...
      if (strlen(fileinfo.name) > suffix_len &&
          0 == strcmp(fileinfo.name + strlen(fileinfo.name) - suffix_len,
                      suffix)) {
        if (!func(path, NULL, fileinfo.name, NULL)) {
    ...
    if (dbdir != NULL) {
      ...
      while (ret == 0) {
        ...
        if (strlen(fileinfo.name) > suffix_len &&
          ...
          the file */
          if (!func(path, dbinfo.name, fileinfo.name, NULL)) {
            os_file_closedir(dbdir);
...
static void delete_file(const std::string &file, bool if_exists = false) {
  if (if_exists && !file_exists(file)) return;
...
*/
static void rename_table_in_prepare(const std::string &datadir,
                                    const std::string &from,
                                    const std::string &to,
                                    const char *extension = 0) {
  if (!extension) {
    static const char *extensions_nonincremental[] = {".ibd", 0};
    static const char *extensions_incremental[] = {".ibd.delta", ".ibd.meta",
                                                   0};
  ...
  if (file_exists(dest)) {
    ...
    if (!file_exists(ren2)) {
      msg("ERROR : File %s was not found, but expected during rename "
          "processing\n",
          ren2.c_str());
      ut_a(0);
    }
...
static ibool prepare_handle_ren_files(const char *datadir, const char *db,
                                      const char *filename, void *) {
  std::string ren_file = std::string(datadir) + "/" + db + "/" + filename;
  if (!file_exists(ren_file)) return TRUE;
...
static ibool prepare_handle_del_files(const char *datadir, const char *db,
                                      const char *filename, void *) {
...
static bool xtrabackup_prepare_func(char **argv) {
  ...
  */
  ...
  const lsn_t target_lsn =
      xtrabackup_incremental ? incremental_to_lsn : metadata_to_lsn;
  ...
  */
  ...
  bool ok = !xtrabackup_incremental || metadata_to_lsn == incremental_lsn;
  if (!ok) {
  ...
  srv_operation =
      xtrabackup_export ? SRV_OPERATION_RESTORE_EXPORT : SRV_OPERATION_RESTORE;
  ...
  if (ok) {
    msg("Last binlog file %s, position %lld", trx_sys.recovered_binlog_filename,
  ...
  if ((srv_start_lsn || fil_space_get(SRV_LOG_SPACE_FIRST_ID)) &&
    ...
    msg("mariabackup: error: "
        "The log was only applied up to LSN " LSN_PF ", instead of " LSN_PF,
        srv_start_lsn, target_lsn);
    ok = false;
  }
#ifdef WITH_WSREP
  ...
  if (ok && xtrabackup_export) ok = (prepare_export() == 0);
...
static bool has_privilege(const std::list<std::string> &granted,
  ...
  for (string_iter i = granted.begin(), e = granted.end(); i != e; ++i) {
    int res = regexec(&priv_re, i->c_str(), 1, tables_regmatch, 0);
...
static bool check_all_privileges() {
  ...
  /* KILL ... */
  if ((!opt_no_lock &&
       (opt_kill_long_queries_timeout || opt_lock_ddl_per_table))
...
bool xb_init() {
  ...
  if (xtrabackup_backup && opt_rsync) {
    ...
    bool have_rsync =
        IF_WIN(false, (system("rsync --version > /dev/null 2>&1") == 0));
    if (!have_rsync) {
      msg("Error: rsync executable not found, cannot run backup with "
          "--rsync\n");
      return false;
    }
  }
...
void handle_options(int argc, char **argv, char ***argv_client,
  ...
  *argv_client = argv;
  *argv_server = argv;
  load_defaults_or_exit(conf_file, xb_server_default_groups, &argc_server,
                        argv_server);
  ...
  load_defaults_or_exit(conf_file, xb_client_default_groups, &argc_client,
                        argv_client);
...
int main(int argc, char **argv) {
  ...
  if (argc > 1) strncpy(orig_argv1, argv[1], sizeof(orig_argv1) - 1);
  ...
  if (THR_THD) (void)pthread_key_delete(THR_THD);
...
static int main_low(char **argv) {
  ...
  if (xtrabackup_stream && !xtrabackup_backup) {
    msg("Warning: --stream parameter is ignored, it only works together with "
        "--backup.");
  }
  ...
  print_version();
  if (xtrabackup_incremental) {
    msg("incremental backup from " LSN_PF " is enabled.", incremental_lsn);
  }
  ...
  /* --prepare */
  if (xtrabackup_prepare && !xtrabackup_prepare_func(argv)) {
    return (EXIT_FAILURE);
  }
  ...
  if (xtrabackup_copy_back || xtrabackup_move_back) {
    ...
    if (!copy_back()) return (EXIT_FAILURE);
  }
