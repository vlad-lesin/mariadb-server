...
static ds_ctxt_t *compress_init(const char *root) {
  ...
  ctxt = (ds_ctxt_t *)my_malloc(sizeof(ds_ctxt_t) + sizeof(ds_compress_ctxt_t),
                                MYF(MY_FAE));
...
static ds_file_t *compress_open(ds_ctxt_t *ctxt, const char *path,
  ...
  file = (ds_file_t *)my_malloc(sizeof(ds_file_t) + sizeof(ds_compress_file_t),
                                MYF(MY_FAE));
  ...
  while (len > 0) {
    ...
    for (i = 0; i <= max_thread; i++) {
      ...
      if (write_uint32_le(dest_file, threads[i].adler) ||
          ds_write(dest_file, threads[i].to, threads[i].to_len)) {
        msg("compress: write to the destination stream "
            "failed.");
        return 1;
      }
...
static comp_thread_ctxt_t *create_worker_threads(uint n) {
  ...
  threads = (comp_thread_ctxt_t *)my_malloc(sizeof(comp_thread_ctxt_t) * n,
                                            MYF(MY_FAE));
  ...
  for (i = 0; i < n; i++) {
    ...
    thd->to = (char *)my_malloc(COMPRESS_CHUNK_SIZE + MY_QLZ_COMPRESS_OVERHEAD,
                                MYF(MY_FAE));
