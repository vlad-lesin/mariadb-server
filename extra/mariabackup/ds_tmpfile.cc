...
static ds_file_t *tmpfile_open(ds_ctxt_t *ctxt, const char *path,
  ...
  fd = create_temp_file(tmp_path, xtrabackup_tmpdir, "xbtemp",
                        O_BINARY | O_SEQUENTIAL, MYF(MY_WME | MY_TEMPORARY));
...
static void tmpfile_deinit(ds_ctxt_t *ctxt) {
  ...
  while (list != NULL) {
    ...
    if (my_seek(tmp_file->fd, 0, SEEK_SET, MYF(0)) == MY_FILEPOS_ERROR) {
      die("my_seek() failed for '%s', errno = %d.", tmp_file->file->path,
          my_errno);
    }
    offset = 0;
    while ((bytes = my_read(tmp_file->fd, (unsigned char *)buf, buf_size,
      ...
      if (ds_write(dst_file, buf, bytes)) {
        die("cannot write to stream for '%s'.", tmp_file->orig_path);
      }
    }
    if (bytes == (size_t)-1) {
