...
#define fil_is_user_tablespace_id(i) (i != 0 && !srv_is_undo_tablespace(i))
...
static inline int msg1(uint thread_num, const char *prefix, const char *fmt,
                       va_list args) {
  int result;
  time_t t = time(NULL);
  char date[100];
  ...
  if (result != -1) {
    if (fmt && fmt[strlen(fmt)] != '\n')
      result =
          fprintf(stderr, "[%02u] %s%s %s\n", thread_num, prefix, date, line);
    else
      result =
          fprintf(stderr, "[%02u] %s%s %s", thread_num, prefix, date, line);
    free(line);
  }
  return result;
}

static inline ATTRIBUTE_FORMAT(printf, 2, 3) int msg(unsigned int thread_num,
                                                     const char *fmt, ...) {
  int result;
  va_list args;
  va_start(args, fmt);
...
static inline ATTRIBUTE_FORMAT(printf, 1, 2) ATTRIBUTE_NORETURN
...
of 2, returns 0.*/
static inline size_t get_bit_shift(size_t value) {
  size_t shift;
...
bytes instead. */
static inline size_t xb_read_full(File fd, uchar *buf, size_t len) {
  size_t tlen = 0;
  size_t tbytes;
