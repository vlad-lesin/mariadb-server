...
static void copy_or_move_dir(const char *from, const char *to, bool copy,
                             bool allow_hardlinks);
...
bool is_path_separator(char c) { return (c == FN_LIBCHAR || c == FN_LIBCHAR2); }
...
static void datadir_node_fill(datadir_node_t *node, datadir_iter_t *it) {
...
static void datadir_node_free(datadir_node_t *node) {
  free(node->filepath);
  free(node->filepath_rel);
  memset(node, 0, sizeof(datadir_node_t));
}

static void datadir_node_init(datadir_node_t *node) {
  memset(node, 0, sizeof(datadir_node_t));
}
...
static datadir_iter_t *datadir_iter_new(const char *path,
                                        bool skip_first_level = true) {
  datadir_iter_t *it;
...
static bool datadir_iter_next_database(datadir_iter_t *it) {
  if (it->dbdir != NULL) {
  ...
  while (os_file_readdir_next_file(it->datadir_path, it->dir, &it->dbinfo) ==
         0) {
    ulint len;

    if ((it->dbinfo.type == OS_FILE_TYPE_FILE && it->skip_first_level) ||
    ...
    len = strlen(it->datadir_path) + strlen(it->dbinfo.name) + 2;
    ...
    snprintf(it->dbpath, it->dbpath_len, "%s/%s", it->datadir_path,
             it->dbinfo.name);
...
static void make_path_n(int n, char **path, ulint *path_len, ...) {
...
static bool datadir_iter_next_file(datadir_iter_t *it) {
  if (it->is_file && it->dbpath) {
    make_path_n(2, &it->filepath, &it->filepath_len, it->datadir_path,
                it->dbinfo.name);

    make_path_n(1, &it->filepath_rel, &it->filepath_rel_len, it->dbinfo.name);
  ...
  while (os_file_readdir_next_file(it->dbpath, it->dbdir, &it->fileinfo) == 0) {
    if (it->fileinfo.type == OS_FILE_TYPE_DIR) {
      continue;
    }

    /* We found a symlink or a file */
    make_path_n(3, &it->filepath, &it->filepath_len, it->datadir_path,
                it->dbinfo.name, it->fileinfo.name);

    make_path_n(2, &it->filepath_rel, &it->filepath_rel_len, it->dbinfo.name,
                it->fileinfo.name);
...
static bool datadir_iter_next(datadir_iter_t *it, datadir_node_t *node) {
  bool ret = true;
...
threads. */
static void datadir_iter_free(datadir_iter_t *it) {
...
static bool datafile_open(const char *file, datafile_cur_t *cursor,
  ...
  strncpy(cursor->rel_path, xb_get_relative_path(cursor->abs_path, FALSE),
          (sizeof cursor->rel_path) - 1);
  cursor->rel_path[(sizeof cursor->rel_path) - 1] = '\0';

  cursor->file = os_file_create_simple_no_error_handling(
      0, cursor->abs_path, OS_FILE_OPEN, OS_FILE_READ_ALLOW_DELETE, true,
      &success);
  if (!success) {
...
static xb_fil_cur_result_t datafile_read(datafile_cur_t *cursor) {
  ...
  if (!os_file_read(IORequestRead, cursor->file, cursor->buf,
                    cursor->buf_offset, to_read)) {
    return (XB_FIL_CUR_ERROR);
  }

  posix_fadvise(cursor->file, cursor->buf_offset, to_read, POSIX_FADV_DONTNEED);
...
static int mkdirp(const char *pathname, int Flags, myf MyFlags) {
  ...
  if (!(parent = strdup(pathname))) return (-1);
...
bool directory_exists(const char *dir, bool create) {
  ...
  if (os_dir == NULL) {
    my_strerror(errbuf, sizeof(errbuf), my_errno);
    msg("Can not open directory %s: %s", dir, errbuf);
...
static bool backup_file_vprintf(const char *filename, const char *fmt,
                                va_list ap) {
  ...
  if (dstfile == NULL) {
    msg("error: Can't open the destination stream for %s", filename);
    goto error;
  }
...
static bool run_data_threads(datadir_iter_t *it, os_thread_func_t func,
                             uint n) {
  ...
  data_threads =
      (datadir_thread_ctxt_t *)malloc(sizeof(datadir_thread_ctxt_t) * n);
...
bool copy_file(ds_ctxt_t *datasink, const char *src_file_path,
  ...
  const char *dst_path = (xtrabackup_copy_back || xtrabackup_move_back)
                             ? dst_file_path
                             : trim_dotslash(dst_file_path);
  ...
  msg(thread_n, "%s %s to %s", xb_get_copy_action(), src_file_path,
      dstfile->path);
  ...
  if (my_rename(src_file_path, dst_file_path_abs, MYF(0)) != 0) {
    if (my_errno == EXDEV) {
      ...
      if (unlink(src_file_path) != 0) {
        ...
        msg("Error: unlink %s failed: %s", src_file_path, errbuf);
      }
      return (ret);
    }
    my_strerror(errbuf, sizeof(errbuf), my_errno);
    msg("Can not move file %s to %s: %s", src_file_path, dst_file_path_abs,
        errbuf);
    return (false);
  }
...
static void read_link_file(const char *ibd_filepath,
...
static const char *tablespace_filepath(const char *ibd_filepath) {
...
static bool copy_or_move_file(const char *src_file_path,
                              const char *dst_file_path, const char *dst_dir,
                              uint thread_n, bool copy = xtrabackup_copy_back) {
  ...
  ret = (copy ? copy_file(datasink, src_file_path, dst_file_path, thread_n)
...
static bool backup_files(const char *from, bool prep_mode) {
  ...
  if (opt_rsync) {
    ...
    if (rsync_tmpfile == NULL) {
      msg("Error: can't create file %s", rsync_tmpfile_name);
      return (false);
    }
  }
  ...
  while (datadir_iter_next(it, &node)) {
    if (!node.is_empty_dir) {
      if (opt_rsync) {
        ret = datafile_rsync_backup(node.filepath, !prep_mode, rsync_tmpfile);
      } else {
      ...
      if (!(ret = backup_file_printf(trim_dotslash(path), "%s", ""))) {
  ...
  if (opt_rsync) {
    ...
    if (!prep_mode && !opt_no_lock) {
      ...
      if (rsync_tmpfile == NULL) {
        msg("Error: can't open file %s", rsync_tmpfile_name);
        ret = false;
        goto out;
      }
...
static lsn_t get_current_lsn(MYSQL *connection) {
  ...
  lsn_t lsn = 0;
  if (MYSQL_RES *res = xb_mysql_query(connection, "SHOW ENGINE INNODB STATUS",
                                      true, false)) {
...
bool backup_finish() {
  ...
  if (mysql_slave_position && opt_slave_info) {
    msg("MySQL slave binlog position: %s", mysql_slave_position);
  }
...
bool ibx_copy_incremental_over_full() {
  const char *ext_list[] = {"frm", "isl", "MYD", "MYI", "MAD", "MAI",
                            "MRG", "TRG", "TRN", "ARM", "ARZ", "CSM",
                            "CSV", "opt", "par", NULL};
  ...
  if (xtrabackup_incremental) {
    ...
    while (datadir_iter_next(it, &node)) {
      ...
      if (node.is_empty_dir || !filename_matches(node.filepath, ext_list)) {
        continue;
      }
      ...
      if (!(ret = copy_file(ds_data, node.filepath, node.filepath_rel, 1))) {
        msg("Failed to copy file %s", node.filepath);
        goto cleanup;
      }
    }
    ...
    snprintf(path, sizeof(path), "%s/" ROCKSDB_BACKUP_DIR,
...
bool apply_log_finish() {
  if (!ibx_cleanup_full_backup() || !ibx_copy_incremental_over_full()) {
    return (false);
  }
...
bool copy_back() {
  ...
  if (!my_stat("ib_logfile0", &stat_arg, MYF(0)) || !stat_arg.st_size) {
    ...
    for (uint i = 0; i <= SRV_N_LOG_FILES_MAX + 1; i++) {
      ...
      snprintf(filename, sizeof filename, "%s/ib_logfile%u", dst_dir, i);
      unlink(filename);
    }
  } else if (!(ret = copy_or_move_file("ib_logfile0", "ib_logfile0", dst_dir,
                                       1))) {
    goto cleanup;
  }
  ds_destroy(ds_data);
  ...
  while (datadir_iter_next(it, &node)) {
    ...
    if (node.is_empty_dir) {
      ...
      snprintf(path, sizeof(path), "%s/%s", mysql_data_home, node.filepath_rel);
      ...
      if (mkdirp(path, 0777, MYF(0)) < 0) {
        char errbuf[MYSYS_STRERROR_SIZE];
        my_strerror(errbuf, sizeof(errbuf), my_errno);
        msg("Can not create directory %s: %s", path, errbuf);
        ret = false;
...
static os_thread_ret_t STDCALL decrypt_decompress_thread_func(void *arg) {
  bool ret = true;
  datadir_node_t node;
  ...
  while (datadir_iter_next(ctxt->it, &node)) {
    ...
    if (!(ret = decrypt_decompress_file(node.filepath, ctxt->n_thread))) {
      goto cleanup;
    }
  }
...
static bool backup_files_from_datadir(const char *dir_path) {
  ...
  while (os_file_readdir_next_file(dir_path, dir, &info) == 0) {
    if (info.type != OS_FILE_TYPE_FILE) continue;

    const char *pname = strrchr(info.name, IF_WIN('\\', '/'));
    if (!pname) pname = info.name;
...
static int rocksdb_remove_checkpoint_directory() {
  xb_mysql_query(mysql_connection,
                 "set global rocksdb_remove_mariabackup_checkpoint=ON", false);
  return 0;
}
...
static bool has_rocksdb_plugin() {
  ...
  if (!first_time || !xb_backup_rocksdb) return has_plugin;
  ...
  if (row) has_plugin = !strcmp(row[0], "1");
...
static void copy_or_move_dir(const char *from, const char *to, bool do_copy,
  ...
  while (datadir_iter_next(it, &node)) {
    ...
    if (!rc) {
      rc = (do_copy ? copy_file(ds_data, from_path, to_path, 1)
                    : move_file(ds_data, from_path, node.filepath_rel, to, 1));
    }
    if (!rc) die("copy or move file failed");
  }
  datadir_iter_free(it);
  datadir_node_free(&node);
}
...
static void rocksdb_lock_checkpoint() {
  ...
  MYSQL_RES *res = xb_mysql_query(
      mysql_connection,
      "SELECT GET_LOCK('mariabackup_rocksdb_checkpoint',3600)", true, true);
...
static void rocksdb_unlock_checkpoint() {
  xb_mysql_query(mysql_connection,
                 "SELECT RELEASE_LOCK('mariabackup_rocksdb_checkpoint')", false,
                 true);
}
...
static void rocksdb_create_checkpoint() {
  MYSQL_RES *result = xb_mysql_query(
      mysql_connection, "SELECT @@rocksdb_datadir,@@datadir", true, true);
  MYSQL_ROW row = mysql_fetch_row(result);
  ...
  snprintf(query, sizeof(query), "SET GLOBAL rocksdb_create_checkpoint='%s'",
...
static void rocksdb_backup_checkpoint() {
  ...
  snprintf(rocksdb_backup_dir, sizeof(rocksdb_backup_dir),
           "%s/" ROCKSDB_BACKUP_DIR, xtrabackup_target_dir);
  bool backup_to_directory =
      xtrabackup_backup && xtrabackup_stream_fmt == XB_STREAM_FMT_NONE;
  if (backup_to_directory) {
    if (my_mkdir(rocksdb_backup_dir, 0777, MYF(0))) {
      die("Can't create rocksdb backup directory %s", rocksdb_backup_dir);
    }
  }
  copy_or_move_dir(rocksdb_checkpoint_dir, ROCKSDB_BACKUP_DIR, true,
                   backup_to_directory);
  rocksdb_remove_checkpoint_directory();
...
*/
static void rocksdb_copy_back() {
  if (access(ROCKSDB_BACKUP_DIR, 0)) return;
  ...
  if (xb_rocksdb_datadir && is_abs_path(xb_rocksdb_datadir)) {
    ...
    snprintf(rocksdb_home_dir, sizeof(rocksdb_home_dir), "%s/%s",
             mysql_data_home,
             xb_rocksdb_datadir ? trim_dotslash(xb_rocksdb_datadir)
                                : ROCKSDB_BACKUP_DIR);
  }
  mkdirp(rocksdb_home_dir, 0777, MYF(0));
  copy_or_move_dir(ROCKSDB_BACKUP_DIR, rocksdb_home_dir, xtrabackup_copy_back,
